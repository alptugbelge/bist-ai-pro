...
const STOCKS = [
  ...
  {c:"PAMEL",n:"Pamel Enerji",s:"Enerji",i:"BIST-TUM"},
...
{"signal":"AL|SAT|BEKLE","confidence":0-100,"summary":"3 cumle kapsamli uzman yorum","chart_reading":"RSI MACD Bollinger destek direnc formasyon","price_action":"fiyat hareketi hacim momentum","fundamental":"sirket temelleri buyume sektor","news_catalyst":"KAP duyurulari haberler","macro":"TCMB faiz enflasyon kur","prediction":"1 hafta 1 ay tahmin","risk":"DUSUK|ORTA|YUKSEK","target_1w":sayi,"target_1m":sayi,"stop_loss":sayi,"entry_price":sayi,"reasons":["r1","r2","r3","r4"],"chart_pattern":"formasyon","momentum":"GUCLU_YUKSELIS|YAVAS_YUKSELIS|YATAY|YAVAS_DUSUS|GUCLU_DUSUS","entry_strategy":"giris stratejisi","position_size":"pozisyon yonetimi"}`,
      tools:[{type:"web_search_20250305",name:"web_search"}],
      messages:[{role:"user",content:`${stock.n} (${stock.c}) - ${stock.s} - ${stock.i}. Bugun kapsamli analiz yap. Son KAP duyurularini, teknik gostergeleri ve makro faktorleri ara.`}]
    });
...
{"bist100":"endeks","direction":"YUKSELIS|DUSUS|YATAY","market_mood":"IYIMSER|NOTR|KARAMSAR","summary":"piyasa 3 cumle","top_opportunities":[{"code":"KOD","name":"isim","signal":"AL","confidence":85,"reason":"sebep","catalyst":"katalizor","sector":"sektor"}],"avoid_list":[{"code":"KOD","name":"isim","reason":"neden"}],"hidden_gems":[{"code":"KOD","name":"isim","reason":"potansiyel","sector":"sektor"}],"hot_sectors":[{"sector":"sektor","reason":"neden"}],"key_news":["haber1","haber2","haber3","haber4"],"risk_level":"DUSUK|ORTA|YUKSEK","weekly_outlook":"bu hafta","daily_strategy":"bugun stratejisi"}`,
      tools:[{type:"web_search_20250305",name:"web_search"}],
      messages:[{role:"user",content:`Bugun ${new Date().toLocaleDateString("tr-TR")} BIST piyasasini kapsamli analiz et.`}]
    });
      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"14px"}}>
        ...
        {tab==="screener"&&(
          <div>
            ...
            <div style={{marginBottom:10,padding:"8px 12px",background:"rgba(245,166,35,.04)",border:"1px solid rgba(245,166,35,.15)",borderRadius:8,fontSize:10,color:"#2e4257"}}>
              Tum BIST hisseleri. Yeni IPO'lar otomatik eklenir. Hisse kodunu sari kutuya yazarak AI analizi baslatin.
            </div>
            <TVScreener/>
          </div>
        )}
              ...
              <div>
                ...
                {aLoad?(
                  <div style={{background:CB,border:`1px solid ${BD}`,borderRadius:12,padding:40,textAlign:"center"}}>
                    ...
                    <div style={{color:"#1e3248",fontSize:10}}>Web aramasi - Grafik okuma - KAP - Haberler - Tahmin</div>
                  </div>
                ):an?(
                  <div style={{display:"grid",gap:10}}>
                    <div style={{background:an.signal==="AL"?"rgba(0,229,180,.05)":an.signal==="SAT"?"rgba(255,61,107,.05)":"rgba(245,166,35,.05)",border:`2px solid ${an.signal==="AL"?G:an.signal==="SAT"?R:Y}40`,borderRadius:12,padding:20}}>
                      <div style={{display:"flex",flexWrap:"wrap",gap:16,alignItems:"center",marginBottom:14}}>
                        ...
                        {[["1H HEDEF",(an.target_1w||"-")+"TL",G],["1A HEDEF",(an.target_1m||"-")+"TL",G],["GIRIS",(an.entry_price||"-")+"TL",Y],["STOP",(an.stop_loss||"-")+"TL",R],["RISK",an.risk||"-",an.risk==="DUSUK"?G:an.risk==="YUKSEK"?R:Y]].map(([l,v,c])=>(
                          <div key={l}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:8}}>
                      {[["Grafik Analizi",an.chart_reading,"#5e9eff"],["Fiyat Hareketi",an.price_action,G],["Temel Analiz",an.fundamental,"#c084fc"],["Haber & KAP",an.news_catalyst,Y],["Makro",an.macro,"#fb923c"],["Tahmin",an.prediction,"#34d399"],["Giris Stratejisi",an.entry_strategy,"#f472b6"],["Pozisyon",an.position_size,"#a78bfa"]].map(([l,v,c])=>(
                    ...
                    {an.reasons&&(
                      <div style={{background:CB,border:`1px solid ${BD}`,borderRadius:10,padding:12}}>
                        <div style={{color:Y,fontSize:10,fontWeight:700,marginBottom:8}}>SINYAL GEREKCELER</div>
              ...
              <div style={{display:"grid",gap:10}}>
                <div style={{background:CB,border:`1px solid ${BD}`,borderRadius:12,padding:16}}>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:10}}>
                    ...
                    <div><div style={{fontSize:9,color:"#2e4257"}}>DUYGU</div><div style={{fontSize:12,color:"#b8ccdc"}}>{(scanResult.market_mood||"-")}</div></div>
                  ...
                  <div style={{background:"rgba(0,229,180,.03)",border:"1px solid rgba(0,229,180,.2)",borderRadius:12,padding:14}}>
                    ...
                    {scanResult.top_opportunities?.map(item=>(
                      <div key={item.code} style={{marginBottom:12,paddingBottom:12,borderBottom:"1px solid rgba(0,229,180,.1)"}}>
                        ...
                        {item.catalyst&&<div style={{color:G,fontSize:9,marginBottom:5}}>Katalizor: {item.catalyst}</div>}
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  {scanResult.hot_sectors&&(
                    <div style={{background:CB,border:`1px solid ${BD}`,borderRadius:10,padding:12}}>
                      <div style={{color:G,fontSize:10,fontWeight:700,marginBottom:8}}>GUCLU SEKTORLER</div>
          ...
          <div>
            ...
            {alerts.length===0?(
              <div style={{textAlign:"center",padding:40,color:"#1e3248"}}>
                <div style={{fontSize:28,marginBottom:8}}>🔔</div>Henuz alarm yok.
              </div>
            ):alerts.map(a=>(
        ...
        {tab==="portfolio"&&(
          <div>
            ...
            {portfolio.length===0?(
              <div style={{textAlign:"center",padding:60,color:"#1e3248"}}>
                <div style={{fontSize:28,marginBottom:8}}>💼</div>Analiz sayfasindan hisse ekleyin.
              </div>
            ):portfolio.map(stock=>(
        ...
        {tab==="abonelik"&&(
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:16,maxWidth:800,margin:"0 auto"}}>
              {[
                {name:"UCRETSIZ",price:"0₺",period:"sonsuza kadar",color:"#2e4257",features:["Haftada 3 analiz","Piyasa listesi","TradingView grafikleri"],locked:["AI tarama","Gizli firsatlar","Alarmlar","AI Asistan"],btn:"Simdi Kullan",btnColor:"#2e4257"},
                {name:"STANDART",price:"49₺",period:"/ ay",color:G,features:["Gunluk 10 analiz","Tum BIST","AI tarama","Alarmlar","Portfoy","AI Asistan"],locked:["Gizli firsatlar"],btn:"Hemen Baslat",btnColor:G,popular:true},
                {name:"PRO",price:"149₺",period:"/ ay",color:Y,features:["Sinirsiz analiz","Tum BIST","Gizli firsatlar","Otomatik tarama","Portfoy optimizasyonu","AI Asistan","7/24 destek"],locked:[],btn:"Pro'ya Gec",btnColor:Y}
              ].map(plan=>(
                <div key={plan.name} style={{background:CB,border:`2px solid ${plan.popular?plan.color:BD}`,borderRadius:16,padding:24,position:"relative"}}>
                  ...
                  <div style={{marginBottom:20}}>
                    {plan.features.map(f=>(<div key={f} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{color:G,fontSize:12}}>✓</span><span style={{color:"#7a94a8",fontSize:11}}>{f}</span></div>))}
                    {plan.locked.map(f=>(<div key={f} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{color:"#1e3248",fontSize:12}}>✗</span><span style={{color:"#1e3248",fontSize:11}}>{f}</span></div>))}
                  </div>
