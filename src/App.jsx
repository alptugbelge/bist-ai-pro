...
function TVChart({ code }) {
  ...
  useEffect(() => {
    ...
    s.innerHTML = JSON.stringify({
      ...
      width:"100%", height:"420", hide_top_toolbar:false
    });
    c.appendChild(s);
    ref.current.appendChild(c);
  }, [code]);
...
function TVNews({ code }) {
  ...
  useEffect(() => {
    ...
    s.innerHTML = JSON.stringify({
      ...
      width:"100%", height:400, colorTheme:"dark", locale:"tr"
    });
    c.appendChild(s);
    ref.current.appendChild(c);
  }, [code]);
...
export default function App() {
  ...
  // Otomatik sabah tarama - seans acilisinda
  useEffect(() => {
    const checkAutoScan = () => {
      const now = new Date();
  ...
  const analyze = async (stock) => {
    ...
    const r = await callAI(
      `Sen dunyanin en iyi BIST borsa uzman yapay zekasisin. Grafik okuma, teknik analiz, temel analiz, KAP haberleri ve makro ekonomi konularinda uzmansın. Web aramasi yaparak en guncel bilgilere eris. Turkce yanit ver. SADECE JSON formatinda yanit ver:
{"signal":"AL|SAT|BEKLE","confidence":0-100,"summary":"3 cumle kapsamli uzman yorum","chart_reading":"RSI MACD Bollinger Band destek direnc seviyeleri formasyon analizi","price_action":"son fiyat hareketi hacim analizi momentum","fundamental":"sirket temelleri buyume potansiyeli sektor durumu PD DD orani","news_catalyst":"son KAP duyurulari onemli haberler fiyat katalizoru","macro":"TCMB faiz enflasyon kur etkisi","prediction":"1 hafta ve 1 ay somut fiyat tahmini ve gerekce","risk":"DUSUK|ORTA|YUKSEK","target_1w":sayi,"target_1m":sayi,"stop_loss":sayi,"entry_price":sayi,"reasons":["r1","r2","r3","r4"],"chart_pattern":"grafik formasyonu","momentum":"GUCLU_YUKSELIS|YAVAS_YUKSELIS|YATAY|YAVAS_DUSUS|GUCLU_DUSUS","entry_strategy":"en iyi giris stratejisi ve zamanlama","position_size":"pozisyon buyuklugu onerisı"}`,
      `${stock.n} (${stock.c}) - ${stock.s} sektoru - ${stock.i}. Simdi BIST:${stock.c} icin kapsamli analiz yap. Son KAP duyurularini, teknik gostergeleri, sektor haberlerini ve makro faktorleri web aramasiyla bul ve analiz et.`
    );
  ...
  const scan = async () => {
    ...
    const r = await callAI(
      `Sen BIST AI analist sistemisisin. Web aramasi yaparak bugunun piyasasini analiz et. SADECE JSON:
{"bist100":"endeks seviyesi","direction":"YUKSELIS|DUSUS|YATAY","market_mood":"ASIRI_IYIMSER|IYIMSER|NOTR|KARAMSAR|ASIRI_KARAMSAR","summary":"bugunun piyasasi 3 cumle","top_opportunities":[{"code":"KOD","name":"isim","signal":"AL","confidence":85,"reason":"detayli sebep","catalyst":"fiyat artis katalizoru","sector":"sektor"}],"avoid_list":[{"code":"KOD","name":"isim","reason":"neden kacinilmali"}],"hidden_gems":[{"code":"KOD","name":"isim","reason":"neden potansiyel var","sector":"sektor"}],"hot_sectors":[{"sector":"sektor","reason":"neden guclu"}],"weak_sectors":[{"sector":"sektor","reason":"neden zayif"}],"key_news":["haber1","haber2","haber3","haber4"],"macro_factors":["faktor1","faktor2","faktor3"],"risk_level":"DUSUK|ORTA|YUKSEK","weekly_outlook":"bu hafta beklenti","daily_strategy":"bugun icin strateji"}`,
      `Bugun ${new Date().toLocaleDateString("tr-TR")} BIST piyasasini kapsamli analiz et. Son KAP duyurulari, BIST100 seviyesi, onemli haberler, makro faktörler. Hem buyuk hem kucuk yuksek potansiyelli hisseleri dahil et.`
    );
  ...
  const sendChat = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          tools:[{type:"web_search_20250305",name:"web_search"}],
          system:"Sen BIST'in en zeki borsa asistanisin. Turkce, net, uzman ve kapsamli yanit ver. Web aramasi yaparak en guncel bilgilere eris.",
    ...
    } catch { setChat(c=>[...c,{r:"ai",t:"Hata olustu, tekrar deneyin."}]); }
    setChatLoad(false);
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,color:"#b8ccdc",fontFamily:"'Courier New',monospace",fontSize:12}}>
      ...
      <div style={{borderBottom:`1px solid ${BD}`,padding:"10px 14px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <div>
            ...
            <div style={{fontSize:8,color:"#1e3248"}}>GERCEK ZAMANLI - AI ANALIZ - OTOMATIK GUNCELLEME</div>
          </div>
        </div>
      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"14px"}}>
        ...
        {tab==="market"&&(
          <div>
            ...
            <div style={{marginBottom:10,padding:"8px 12px",background:"rgba(0,229,180,.04)",border:"1px solid rgba(0,229,180,.15)",borderRadius:8,fontSize:10,color:"#2e4257"}}>
              Sistem her sabah 10:10'da otomatik tarama yapar. Herhangi bir hisse icin sarı kutuya kod yazin + Enter.
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:8}}>
              {filtered.map(stock=>(
                <div key={stock.c} onClick={()=>analyze(stock)}
                  style={{background:CB,border:`1px solid ${signals[stock.c]==="AL"?"rgba(0,229,180,.4)":signals[stock.c]==="SAT"?"rgba(255,61,107,.4)":BD}`,borderRadius:10,padding:12,cursor:"pointer",transition:"all .15s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor=Y}
                  onMouseLeave={e=>e.currentTarget.style.borderColor=signals[stock.c]==="AL"?"rgba(0,229,180,.4)":signals[stock.c]==="SAT"?"rgba(255,61,107,.4)":BD}>
        ...
        {tab==="screener"&&(
          <div>
            <div style={{color:Y,fontSize:10,letterSpacing:2,marginBottom:8}}>TUM BIST - 600+ HİSSE CANLI SCREENER</div>
            <div style={{marginBottom:10,padding:"8px 12px",background:"rgba(245,166,35,.04)",border:"1px solid rgba(245,166,35,.15)",borderRadius:8,fontSize:10,color:"#2e4257"}}>
              Tum BIST hisseleri burada. Yeni IPO'lar otomatik eklenir. Herhangi bir hisse kodunu Piyasa sekmesindeki sarı kutuya yazarak AI analizi baslatin.
            </div>
            <TVScreener/>
          </div>
        )}
              ...
              <div>
                ...
                {aLoad?(
                  ...
                  <div style={{display:"grid",gap:10}}>
                    <div style={{
                      background:an.signal==="AL"?"rgba(0,229,180,.05)":an.signal==="SAT"?"rgba(255,61,107,.05)":"rgba(245,166,35,.05)",
                      border:`2px solid ${an.signal==="AL"?G:an.signal==="SAT"?R:Y}40`,
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:16,alignItems:"center",marginBottom:14}}>
                        ...
                        {[["1H HEDEF",(an.target_1w||"-")+"TL",G],["1A HEDEF",(an.target_1m||"-")+"TL",G],["GIRIS",(an.entry_price||"-")+"TL",Y],["STOP",(an.stop_loss||"-")+"TL",R],["RISK",an.risk||"-",an.risk==="DUSUK"?G:an.risk==="YUKSEK"?R:Y],["MOMENTUM",(an.momentum||"-").replace(/_/g," "),Y]].map(([l,v,c])=>(
                          <div key={l}>
                    ...
                    <div style={{marginTop:4}}>
                      <div style={{color:"#fb923c",fontSize:10,fontWeight:700,marginBottom:6,letterSpacing:1}}>SON HABERLER & KAP</div>
          ...
          <div>
            ...
            {scanLoad?(
              <div style={{textAlign:"center",padding:60}}>
                ...
                <div style={{color:"#1e3248",fontSize:10}}>KAP haberleri - Teknik gostergeler - Makro faktorler</div>
              </div>
            ):scanResult?(
              <div style={{display:"grid",gap:10}}>
                <div style={{background:CB,border:`1px solid ${BD}`,borderRadius:12,padding:16}}>
                  <div style={{display:"flex",gap:16,flexWrap:"wrap",marginBottom:10}}>
                    ...
                    <div><div style={{fontSize:9,color:"#2e4257"}}>DUYGU</div><div style={{fontSize:13,fontWeight:700,color:"#b8ccdc"}}>{scanResult.market_mood?.replace(/_/g," ")||"-"}</div></div>
                  ...
                  {scanResult.daily_strategy&&<p style={{color:Y,fontSize:11,margin:"0 0 6px",fontStyle:"italic"}}>Strateji: {scanResult.daily_strategy}</p>}
                  ...
                  <div style={{background:"rgba(0,229,180,.03)",border:"1px solid rgba(0,229,180,.2)",borderRadius:12,padding:14}}>
                    ...
                    {scanResult.top_opportunities?.map(item=>(
                      <div key={item.code} style={{marginBottom:12,paddingBottom:12,borderBottom:"1px solid rgba(0,229,180,.1)"}}>
                        ...
                        {item.catalyst&&<div style={{color:G,fontSize:9,opacity:.8,marginBottom:5}}>Katalizor: {item.catalyst}</div>}
          ...
          <div>
            ...
            {alerts.length===0?(
              <div style={{textAlign:"center",padding:40,color:"#1e3248"}}>
                ...
                Henuz alarm yok. Yukardaki formdan ekleyin.
              </div>
            ):alerts.map(a=>(
            ...
            <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:8,paddingBottom:8}}>
              {chat.length===0&&(
                <div style={{textAlign:"center",padding:40,color:"#1e3248"}}>
                  ...
                  {["THYAO bu hafta yukselir mi?","Hangi sektor en guclu?","Kucuk hisselerde nasil para kazanilir?","Portfoyum icin strateji oner"].map(q=>(
              ...
              {chatLoad&&(
                <div style={{display:"flex",justifyContent:"flex-start"}}>
                  <div style={{background:CB,border:`1px solid ${BD}`,borderRadius:12,padding:"10px 14px",color:Y,fontSize:11}}>
                    Arastiriyor ve analiz ediyor...
                  </div>
                </div>
              )}
              <div ref={chatEnd}/>
      ...
      <div style={{textAlign:"center",padding:12,color:"#0f1e2d",fontSize:9,borderTop:`1px solid ${BD}`,marginTop:16}}>
        BIST AI PRO - Yatirim tavsiyesi degildir. Tum yatirim kararlari yatirimciya aittir. Sermaye kaybi riski mevcuttur.
      </div>
    </div>
  );
}
